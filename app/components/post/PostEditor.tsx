...
function ImageUpload() {
  ...
  return portalRoot && createPortal(
    <Popover className="absolute top-6 right-6">
      <Popover.Button title='Upload image or video'>
      ...
      <Popover.Panel className={`absolute top-full right-0 z-10 w-72 shadow-md ${cardCN}`}>
            ...
            <>
              ...
              {urls.map(url => {
                if (url.startsWith('data:image')) {
                  return <img key={url} className='my-4' src={url} alt="" />
                }
                if (url.startsWith('data:video')) {
                  return <video key={url} className="my-4" src={url} controls />
                }
                return null
              })}
              <button
